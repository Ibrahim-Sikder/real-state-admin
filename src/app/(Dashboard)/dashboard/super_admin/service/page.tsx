    ...
    <Box sx={{ width: "100%", p: 2 }}>
      ...
      >
        <Typography variant="h6" component="h1" sx={{ fontWeight: 500 }}>
          Services ({serviceData?.data?.services?.length})
        </Typography>
          ...
          <TableBody>
            {serviceData?.data?.services?.map((service: TService, index: number) => (
              <TableRow key={service._id}>
                ...
                <TableCell sx={cellStyle}>{service.title}</TableCell>
                <TableCell sx={cellStyle}> {parse(truncateText((service.description || ""), 50))}</TableCell>
